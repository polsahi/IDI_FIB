...
void MyGLWidget::modelTransformQuadrat(glm::vec3 posicio)
{
  ...
  TG = glm::rotate(TG, beta, glm::vec3(0, 0, 1)); 
...
void MyGLWidget::modelTransformCavall(glm::vec3 posicio)
{
  ...
  TG = glm::rotate(TG, alpha, glm::vec3(0, 0, 1));
  glUniformMatrix4fv(TGLoc, 1, GL_FALSE, &TG[0][0]); 
}
...
//  glViewport (0, 0, ample, alt);
  ...
  for (int i = 0; i < 8; ++i) {
    for (int j = 0; j < 8; ++j) {
      ...
      modelTransformQuadrat(glm::vec3(i/4.0 - .875, j/4.0 - .875, 0.0));
  ...
  if (moved) {
    modelTransformCavall(glm::vec3(0.199,0.020,0.0));
    ...
    modelTransformCavall(glm::vec3(0.199,0.02,0.0));
    glBindVertexArray(VAOCavall);
...
void MyGLWidget::creaBuffersCavall()
{
  ...
  Vertices[ 1] =  glm::vec3(0.1, 1.6, 0.0); // Vèrtex més abaix
  ...
  Vertices[20] =  glm::vec3(1.3, 1.7, 0.0);  // Vèrtex més a la dreta      Amplada = 1.7 - 0.1 = 1.6  Real = 1.6*0.08 = .128
  Vertices[21] =  glm::vec3(1.3, 1.7, 0.0);               // Alçada 2.6 - 0.1 = 2.5
  Vertices[22] =  glm::vec3(0.8, 0.4, 0.0);               // Volem que faci 0.2 per tant s'ha d'esclar en 0.08
  ...
  Vertices[38] =  glm::vec3(2.6, 0.6, 0.0);   // Vèrtex més alt  
  Vertices[39] =  glm::vec3(1.6, 1.0, 0.0);
  Vertices[40] =  glm::vec3(2.2, 0.7, 0.0);       
  Vertices[41] =  glm::vec3(1.4, 0.1, 0.0);   // Vèrtex més cap a l'esquerra
